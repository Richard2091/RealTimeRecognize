...
from camera import CameraManager, CameraFactory, CameraState, LocalCamera
...
class CameraHandler(QObject):
    ...
    def detect_cameras(self, max_id: int = 3) -> dict:
        """
        检测可用的本地摄像头
    ...
    def _on_frame_timer(self):
        ...
        if self.camera_manager.is_reading():
            ...
            if ret and frame is not None:
                # print(f"✓ 成功读取帧: {frame.shape}")  # 注释掉持续输出
                # 通知主窗口处理帧
                self.main_window.process_camera_frame(frame)
            else:
    ...
    def _on_state_changed(self, old_state: str, new_state: str):
        ...
        # 更新UI状态
        if hasattr(self.main_window, 'update_camera_status'):
            self.main_window.update_camera_status(new_state)
