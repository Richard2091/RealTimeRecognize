...
class CameraController(QObject):
    ...
    def __init__(self):
        ...
        self._state = CameraStatus.CLOSED
    ...
    def open_camera(self) -> bool:
        """打开摄像头"""
        if self._state != CameraStatus.CLOSED:
            self._notify_error(f"无法打开摄像头，当前状态: {self._state}")
            return False

        try:
            # 状态转换：closed -> opening
            self._change_state(CameraStatus.OPENING)
            ...
            if self._camera is None:
                ...
                self._change_state(CameraStatus.CLOSED)
                return False

            # 状态转换：opening -> opened
            self._change_state(CameraStatus.OPENED)
            ...
            self._change_state(CameraStatus.ERROR)
            return False
    ...
    def start_reading(self) -> bool:
        """开始读取帧"""
        if self._state != CameraStatus.OPENED:
        ...
        try:
            ...
            # 状态转换：opened -> reading
            self._change_state(CameraStatus.READING)
            print("✓ 开始读取帧")
    ...
    def stop_reading(self) -> bool:
        ...
        try:
            ...
            # 状态转换：reading -> opened
            if self._state == CameraStatus.READING:
                self._change_state(CameraStatus.OPENED)
    ...
    def read_frame(self) -> tuple:
        ...
        if self._state not in [CameraStatus.OPENED, CameraStatus.READING]:
            return False, None
    ...
    def is_opened(self) -> bool:
        """检查摄像头是否已打开"""
        return self._state in [CameraStatus.OPENED, CameraStatus.READING]
